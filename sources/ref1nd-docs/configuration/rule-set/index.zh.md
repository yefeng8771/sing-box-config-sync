...
对于远程规则集，下载内容存储于该路径，缓存元数据存储于 `cache.db`。留空时，规则集内容本身存储于 `cache.db`。

远程规则集中与 `initial_path` 冲突。
...
需要使用 `cache.db` 缓存规则集。仅在启动时没有可用的规则集缓存时读取一次，使启动不被初始下载阻塞。启动后规则集仍会立即在后台更新。
